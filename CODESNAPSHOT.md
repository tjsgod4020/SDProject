...
{
    /// CSV 변경을 감지해 모든 DataTableConfig를 자동 동기화.
    /// - RowTypeName을 AssemblyQualifiedName으로 자동 채움
    /// - [DataTableId("Id")] 또는 클래스명==Id, Id+"Row" 규칙으로 매핑
    /// - 메뉴로 CSV 폴더 선택/토글/수동 동기화 지원
    public sealed class DataTableConfigAutoSync : AssetPostprocessor
    {
        ...
        private static void SetCsvFolder()
        {
            ...
            if (!string.IsNullOrEmpty(sel))
            {
                ...
                if (!sel.StartsWith(proj)) { Debug.LogError("선택한 폴더는 프로젝트 Assets 내부여야 합니다."); return; }
                var rel = sel.Substring(proj.Length).Replace('\\', '/');
                if (!rel.StartsWith("Assets/")) rel = "Assets/" + rel.TrimStart('/');
                if (!AssetDatabase.IsValidFolder(rel)) { Debug.LogError($"유효하지 않은 폴더: {rel}"); return; }
            ...
            ref int filledTypes)
        {
            ...
            // 프로젝트마다 이름이 다를 수 있으니 필요하면 여기만 수정.
            ...
            foreach (var p in csvPaths)
            {
                ...
                // 우선순위: [Attr] → 클래스명==Id → 클래스명==Id+"Row"
                Type rowType = null;
                if (!byAttr.TryGetValue(id, out rowType) &&
                    !byName.TryGetValue(id, out rowType) &&
                    !byNameRow.TryGetValue(id, out rowType))
                {
                    // 매칭 실패 시 경고만 남기고 빈 값으로 둠 (로더가 스킵/경고 처리)
                    Debug.LogWarning($"[DataTable] No row type matched for Id '{id}' (Config: {cfg.name}). " +
                                     $"Add [DataTableId(\"{id}\")] or rename class to '{id}' or '{id}Row'.");
                }
                ...
                elem.FindPropertyRelative("RowTypeName")?.SetString(rowType?.AssemblyQualifiedName ?? string.Empty);
                if (rowType != null) localFilled++;
            }
            ...
            out Dictionary<string, Type> byNameRow)
        {
            ...
            var iface = typeof(SD.DataTable.IHasStringId);
...
{
    /// <summary>
    /// CSV 파일명(확장자 제외) = 테이블 ID 와 매칭하기 위한 힌트.
    /// 예) [DataTableId("CardData")] public class CardData : IHasStringId { ... }
    /// </summary>
